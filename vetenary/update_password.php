...
$sql=mysqli_query($conn,"select * from vetenary where password='$op' and email='".$_SESSION['vetenary']."'");
...
if($r==true)
{
	...
	if($np==$cp)
	{
	$sql=mysqli_query($conn,"update vetenary set password='$np' where email='".$_SESSION['vetenary']."'");
	
	$err="<font color='blue'><h3>Password updated successfully</h3></font>";
	}
	else
	{
